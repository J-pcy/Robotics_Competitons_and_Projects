...
/*********************************   读取AD值	**************************************/
static  int AD_acceler_max,AD_acceler_min;
static  int AD_gyro_max,AD_gyro_min;
static  int AD_acceler_totle,AD_gyro_totle,AD_acceler,AD_gyro;
//卡尔曼滤波，就算倾角和角加速度
...
//        OutData[2]=acceler_val;
	...
	if(acceler_val<-4416+offset)acceler_val=-4416+offset;
	if(acceler_val>2664+offset)acceler_val=2664+offset;

	acceler_val-=(-1469.22+offset);			//前倾，值小，后倒，值大

	acceler_val*=0.0054001;		//系数换算：（5.03/1024）/(0.8);0.0061401
//	acceler_val=asin(acceler_val);

	gyro_val-=(163.12+off_gy);				//前倾，值增大，后倒，减小
	gyro_val=gyro_val*(-1);
	gyro_val*=(0.081/K_gy);			//角速度系数：(3.14/180)* （5.03/1024/(0.00068*A)）=0；A=5
...
void setup()
{
  ...
  pinMode(Moto_RA,OUTPUT);
  pinMode(Moto_RB,OUTPUT);
  pinMode(Moto_LA,OUTPUT);
  pinMode(Moto_LB,OUTPUT);
