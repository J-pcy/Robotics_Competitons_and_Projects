...
#define Moto_Out_Max 99
#define Moto_Out_Min -99
...
static float DIR_CONTROL_P=1.5;
...
static unsigned char black_error=55;


unsigned char Limit=59;
...
//  }
//  else if(PixelAverageValue>=20) 
//  {
//    Limit=PixelAverageValue-8;    
//  }
//  else
/*  if(PixelAverageValue<=20)
  {
    Limit=PixelAverageValue-4;  
  }  */    
  for(i=0;i<128;i++)
  {
    if(Pixel[i]<=Limit)
...
//    black_Limit=Black_Limit-24;
  ...
  if(Pixel1[63]==1&&Pixel1[64]==1)
  {
    black=black_Limit;
    Car_speed_set=Car_speed_set-0.02;
    if(Car_speed_set<=0)
    {
      Car_speed_set=0;
    }   
...
/*  if(black<=Black_Limit+3&&black>=Black_Limit-3)
  {
...
//  LCD_Init();
  ...
  {  
    ...
    if(j%3==0)
    {
      ...
      if(PORTB_PB2==0) //使用模式一下的调整速度功能
      {
      Dly_ms(10);
      if(PORTB_PB2==0)
      {
        Car_speed_select=Car_speed_select+0.5;
        if(Car_speed_select>4)
        {
          Car_speed_select=0;
        }
      }
    ...
    LCD_Show_Number (35,0,C_Gyro_y); //陀螺仪的值
    LCD_write_english_string(0,1,"A:");
    LCD_Show_Number (35,1,C_Z);
    LCD_write_english_string(0,2,"Speed:");
    LCD_Show_Number (35,2,Car_speed_select*10);
    LCD_Show_Number (0,3,black);
    LCD_Show_Number (0,4,black_left);
    LCD_Show_Number (35,4,black_right);
    LCD_write_english_string(30,3,"LMI:");
    LCD_Show_Number (50,3,Limit);           
   // LCD_Show_Number (40,4,black_right);
...
/*      LCD_write_english_string(0,0,"AD_az:");
      ...
      LCD_Show_Number (0,2,C_Gyro_y);   
      LCD_Show_Number (0,3,PixelAverageValue);  */
    
  }
  
}

/*#pragma CODE_SEG __NEAR_SEG NON_BANKE
