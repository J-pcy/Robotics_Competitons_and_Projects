...
static float DIR_CONTROL_P=1.2;
...
unsigned char black_right=0;
unsigned char black_left=0;
unsigned char black=0,blackOld_1=0,blackOld_2=0,blackOld_3=0,blackOld_4=0,blackOld_5=0;
unsigned char black_error=62;
unsigned char black_slope=0;
...
void MotorOutput()
{
  ...
  fLeft = g_fAngleControlOut - g_fSpeedControlOut + g_fDirectionControlOut+16;
...
void Binarization(void)//二值化的程序
{
  ...
  else if(PixelAverageValue>=30) 
  {
    Limit=PixelAverageValue-12;  
  }
  else 
  {
    Limit=PixelAverageValue-6;    
  }
  for(i=0;i<128;i++)
  {
    if(Pixel[i]<=Limit)
...
void black_extract(void)              
{
  ...
  if(black_left!=0&&black_right!=0)
  {
    blackOld_5=blackOld_4;
    blackOld_4=blackOld_3;
    blackOld_3=blackOld_2;
    blackOld_2=blackOld_1;
    blackOld_1=black;
...
{
  black_slope=(black-blackOld_5)*0.03;
  if(black_slope<0)
  {
    black_slope=-black_slope;
  }
  if(black<=black_Limit&&black>black_Limit-32)
  {
    g_fDirectionControlOut=DIR_CONTROL_P*black_slope*(black_Limit-black); 
  } 
  else if(black<=black_Limit-32)
  {
    g_fDirectionControlOut=DIR_CONTROL_P*black_slope*(black_Limit-black); 
  }
  else if(black>=black_Limit&&black<black_Limit+32)
  {
    g_fDirectionControlOut=DIR_CONTROL_P*black_slope*(black_Limit-black); 
  }
  else if(black>=black_Limit+32)
