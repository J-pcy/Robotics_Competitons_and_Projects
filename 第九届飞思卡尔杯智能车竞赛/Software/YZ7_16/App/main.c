...
static float DIR_CONTROL_D=200;//50,200
...
unsigned int DIR_CONTROL_P_c=80;
unsigned int DIR_CONTROL_P_div=50;
int js_flag=0;
...
void IO_init()
{
  ...
  gpio_init(PTD10,GPI,LOW);
  gpio_init(PTD11,GPI,LOW);
  gpio_init(PTD13,GPI,LOW);
}
...
{
  uint8 i=0,j=0;
  uint8 low=0,leftlow=0,rightlow=0;
  for(i=44;i<49;i++)
  {
    if((Buffer[i][Line_Center[i]-2]==1)&&(Buffer[i][Line_Center[i]+2]==1))
    {
      if(leftlow<5) 
      {
        low=0;
        for(j=Line_Center[i]-4;j>Line_Center[i]-30;j--)
        {
          if(Buffer[i][j]==0)  low++;
          if((low)&&(Buffer[i][j]==1))
          {
...
void DirectionControl(void)
{
  ...
  DIR_CONTROL_P=string_error*string_error/DIR_CONTROL_P_div+DIR_CONTROL_P_c;
...
//    LCD_clear();
    ...
    if(k%6==0)
    {
      ...
      {
        Car_speed_select=Car_speed_select+1;
        while(key_check(KEY_D) == 0);
      }
      if(key_check(KEY_L)==0)
      {
        Car_speed_select=Car_speed_select-1;
        while(key_check(KEY_L) == 0);
      }
    }
    else if(k%6==1)
    {
      LCD_write_english_string(0,5,"C_Z   mode");//目前调速
      if(key_check(KEY_D)==0)
      {
        C_Z=C_Z+500;
        while(key_check(KEY_D) == 0);
    ...
    {
      LCD_write_english_string(0,5,"Turn mode1");
      if(key_check(KEY_D)==0)
      {
        DIR_CONTROL_P_div=DIR_CONTROL_P_div+10;
        while(key_check(KEY_D) == 0);
      }
      if(key_check(KEY_L)==0)
      {
    ...
    {
      LCD_write_english_string(0,5,"Turn mode2");
      if(key_check(KEY_D)==0)
      {
        DIR_CONTROL_P_c=DIR_CONTROL_P_c+5;
        while(key_check(KEY_D) == 0);
      }
      if(key_check(KEY_L)==0)
      {
    ...
    {
      LCD_write_english_string(0,5,"Turn mode3");
      if(key_check(KEY_D)==0)
      {
        DIR_CONTROL_D=DIR_CONTROL_D+20;
        while(key_check(KEY_D) == 0);
      }
      if(key_check(KEY_L)==0)
      {
    ...
    {
      LCD_write_english_string(0,5,"START mode");
      if(key_check(KEY_L)==0)
      {
        START_flag=1;
        EnableInterrupts;
    ...
    LCD_write_english_string(0,0,"AZ:");
    LCD_Show_Number(35,0,(unsigned int)AD_az);
    LCD_write_english_string(0,1,"A:");
    LCD_Show_Number(35,1,(unsigned int)C_Z);
    LCD_write_english_string(0,2,"Speed:");
    LCD_Show_Number(35,2,(unsigned int)Car_speed_select);
//    LCD_write_english_string(0,3,"Turnd:");
    LCD_Show_Number(0,3,(unsigned int)DIR_CONTROL_P_div);
    LCD_Show_Number(35,3,(unsigned int)DIR_CONTROL_P_c);
    LCD_write_english_string(0,4,"TurnD:");
    LCD_Show_Number(35,4,(unsigned int)DIR_CONTROL_D);
...
//      printf("%d\t%d\n",M_PACNT_left,M_PACNT_right);
    ...
    if(time_q>=1000)
    {
      ...
      if(Car_speed_set>=Car_speed_select*0.1)
      {
        Car_speed_set=Car_speed_select*0.1;
      }
    }
  } 
  else if(PIT_flag0==1)
