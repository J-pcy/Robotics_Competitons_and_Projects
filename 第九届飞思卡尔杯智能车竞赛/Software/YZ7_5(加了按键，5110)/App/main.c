...
void PORTB_IRQHandler1(void);        //PORTB1端口中断服务函数
void key_handler1(void);             //按键按下的测试中断复位函数
void key_handler2(void);             //按键按下的测试中断复位函数
void key_handler3(void);             //按键按下的测试中断复位函数

void PORTB_IRQHandler2(void);        //PORTB1端口中断服务函数
void PORTB_IRQHandler3(void);        //PORTB1端口中断服务函数
...
#define Motor_Out_Dead_Val 48
#define Front_Line 50
...
float Car_speed_select=2.2;
float Car_speed_turn=1.2;
...
static float DIR_CONTROL_P=80;  
//static float DIR_CONTROL_I=0;
static float DIR_CONTROL_D=50;
...
void PORTD_IRQHandler(void)
{
    ...
    if(w%3==0)
    {
      LCD_write_english_string(0,5,"speed mode");
      
      if(PORTD_ISFR & (1 << 11))           //PTD7 触发中断
    {
        PORTD_ISFR  = (1 << 11);        //写1清中断标志位
    ...
    {
    LCD_write_english_string(0,5,"C_Z mode");
    ...
    {
    LCD_write_english_string(0,5,"Lmi mode");
     if(PORTD_ISFR & (1 << 11))           //PTD7 触发中断
    {
        PORTD_ISFR  = (1 << 11);        //写1清中断标志位
...
{
  js_flag=0;
  js_flag=(Line_Center[29]-Line_Center[31])+(Line_Center[28]-Line_Center[32])+(Line_Center[27]-Line_Center[33])+(Line_Center[26]-Line_Center[34]);
}
...
void DirectionControl(void)
{
  ...
  DIR_CONTROL_P=string_error*string_error/40+70;
...
{    
  ...
  //按键
   port_init(PTD10, ALT1 | IRQ_FALLING | PULLUP );          //初始化 PTB1 管脚，复用功能为GPIO ，下降沿触发中断，上拉电阻
   port_init(PTD11, ALT1 | IRQ_FALLING | PULLUP );          //初始化 PTB2 管脚，复用功能为GPIO ，下降沿触发中断，上拉电阻
  port_init(PTD13, ALT1 | IRQ_FALLING | PULLUP );          //初始化 PTB3 管脚，复用功能为GPIO ，下降沿触发中断，上拉电阻
 //按键
  ...
  // LCD_init();
  // LCD_clear();
   //LCD_write_english_string(10,0,"Welcome yyy");
 //  LCD_set_XY(45, 3);
  // LCD_write_char( 0x30+2);   //显示十位数
  // LCD_write_char( 0x30+0);   //显示十位数
  // LCD_write_char( 0x30+6);   //显示个位数
 //5110  //5110 //5110
  
    LCD_write_english_string(0,0,"G:");
    LCD_set_XY(35, 0);
    LCD_write_char( 0x30+0);  // C_Gyro_y
    LCD_write_english_string(0,1,"A:");
    LCD_set_XY(35, 1);
    LCD_write_char( 0x30+0);  //C_Z
  
    LCD_write_english_string(0,2,"Speed:");
    LCD_set_XY(35, 2);
    LCD_write_char( 0x30+Car_speed_select*10); //
     LCD_set_XY(0, 3);
    LCD_write_char( 0x30+0);// black
    LCD_set_XY(0,4);
    LCD_write_char( 0x30+0);// black_left
    LCD_set_XY(35,4);
    LCD_write_char( 0x30+0);//black_right
    
    LCD_write_english_string(30,3,"LMI:");
    LCD_set_XY(50,3);
    LCD_write_char( 0x30+0);//Limit
  ...
  set_vector_handler(PORTD_VECTORn ,PORTD_IRQHandler);    //设置LPTMR的中断复位函数为 PORTA_IRQHandler
  //set_vector_handler(PORTD_VECTORn ,PORTD_IRQHandler2);    //设置LPTMR的中断复位函数为 PORTA_IRQHandler
  //set_vector_handler(PORTD_VECTORn ,PORTD_IRQHandler3);    //设置LPTMR的中断复位函数为 PORTA_IRQHandler
...
//    gpio_set(PTD1,LOW);
    if(time_q>=1000)
    {
      if((js_flag>=5)||(js_flag<=-5))
      {  
         Car_speed_set=Car_speed_set-0.005;
         if(Car_speed_set<=Car_speed_turn)
        {
          Car_speed_set=Car_speed_turn;
        }
      }
    }
