...
static float Angle_Control_P=9;         //20           /3             /9
static float Angle_Control_D=0.18;         //0.41        /0.11          /0.18
...
int running_flag=0;
int degree_now=0,speed_now=0;
int speed_last_time=0,speed_l_last_time=0;
int degree_last_time=0,degree_l_last_time=0;
int stater_flag=0;
int high_flag=0;
int degree_max=0;
int dir=0;
    ...
    else if(PORTB_PB1==0)  //本次采用
    {
			...
			while(PORTB_PB2!=0)
			{
				...
				while(running_flag==2) 
				{
					...
					{
					if(k==0)  Moto(1,100);
					if(k>0&&k<20)Dly_ms(20);
					if(k==20) Moto(0,0);
					if(k>20&&k<25) Dly_ms(20);
					if(k==25)Moto(0,100);
					if(k>27&&k<=45) Dly_ms(20);
					if(k==46)Moto(0,0);
					if(k>46&&k<=49) Dly_ms(20);
					k++;
					if(k>50) k=0;
					}
					...
					else if(flip_flag==1) 
					{
						...
						{  
						if(j==0)  Moto(0,100);
						if(j>0&&j<20)Dly_ms(20);
						if(j==20) Moto(0,0);
						if(j>20&&j<25) Dly_ms(20);
						if(j==25)Moto(1,100);
						if(j>27&&j<=45) Dly_ms(20);
						if(j==46)Moto(0,0);
						if(j>46&&j<=49) Dly_ms(20);
						j++;
						if(j>50) j=0;
						}
					}*/
...
interrupt VectorNumber_Vpit0 void  timer_inte0(void) 
{
  ...
  else if(running_flag==2)
  {
	 read_AD();
     angle_calculate();
     degree_now=Angle_Z; //进行位置获取
     speed_now=Angle_G;  //进行速度获取

     if(speed_now<100&&speed_now>-100)
     {
       if(degree_now>95&&degree_now<170) dir=1;
       else if(degree_now<-95&&degree_now>-170) dir=0;
       else if(degree_now<95&&degree_now>10) 
       {
		  dir=3;
          high_flag=1;
       }
       ...
       else if(degree_now<15&&degree_now>-15) running_flag=1;

     }                                       
     else if((speed_now<00&&speed_now>100)||(speed_now>-200&&speed_now<-100)) 
     {
        // if(degree_now<45&&degree_now>-45) flip_flag=1;
     }
	 else dir=3;
